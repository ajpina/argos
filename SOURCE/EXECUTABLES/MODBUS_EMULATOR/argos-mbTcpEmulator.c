...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * 
 * This program is a MODBUS Tcp emulator server. It will accept an
 * arbitrary number of incoming connections and will serve up random
 * data in MODBUS format ...
 *
 * Command line paramaters are 
 *   -d  Run in debug mode and do not daemonize
 *   -j  filename of json configuration file 
 *
 * To stop either version of the program use the command
 *    kill -HUP `pidof argos-mbTcpEmulator`
 *
 * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 */
...
uint16_t twoBytes;
   ...
   if ( debugMode ) ARGOS_PRINTF("Argos-mbTcpEmulator TICKTOCK <%ld>\n",tickCounter);
...
void loopStopper( int sig ) {
  if ( debugMode ) ARGOS_PRINTF("Argos-mbTcpEmulator stopping ...\n");
  forever = 0;
}
 ...
 */
   ...
   if ( i != 0 ) {
      ARGOS_PRINTF("MODBUS_SET_DEBUG did not work!\n");
      exit(0);
   }
...
/* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * When we are told to shutdown ( via SIGHUP ) we clean up and exit
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 */
